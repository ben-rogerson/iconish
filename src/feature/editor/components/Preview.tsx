      ...
      <div className="grid gap-3">
        ...
        <div className="grid-cols-[minmax(0,_0.25fr)_minmax(0,_1fr)] rounded border md:grid">
          <div className="relative grid place-items-center p-4">
            <div className="px-[22%] py-[10%]">{iconBarrier}</div>
          </div>
          <div className="relative grid gap-4 py-6 pr-8">
            <Upload onUpload={handleOnUpload} className="w-full" />
            <div className="grid items-center">
              <input
                ref={ref}
                type="text"
                className="w-full bg-transparent py-3 text-[--input-text] placeholder-[var(--text-muted)] focus:outline-0"
                placeholder="Or paste a svg here"
                onKeyDown={e => {
                  if (e.key !== 'Enter') return
